...
class TradeTransfer extends Contract {
    ...
    async CreateExecutionTrade(ctx, id, ownerSell, ownerBuy, tradeType, value, price, creationDate) {
        ...
        ctx.stub.putState(id, Buffer.from(JSON.stringify(executionTrade)));
    ...
    async ExecutedTrade(ctx, idSell, idBuy, price) {
        ...
        // get sell and buy trades from id
        ...
        {
            // calculate the value executed
            value = tradeBuy.value;

            tradeSell.value = tradeSell.value - tradeBuy.value;

            // update sell trade
            this.UpdateTrade(ctx, tradeSell.Id, tradeSell.Owner, tradeSell.TradeType, value, price, new Date().toISOString());
        ...
        else if (tradeSell.value === tradeBuy.value) 
        {
            ...
            // update buy trade
            this.UpdateTrade(ctx, tradeBuy.Id, tradeBuy.Owner, tradeBuy.TradeType, value, price, new Date().toISOString());            
        }

        // create a new executed trade
        return this.CreateExecutionTrade(ctx, uuidv4(), tradeSell.Owner, tradeBuy.Owner, 'Executed', value, price, new Date().toISOString());
    }
}
